...
class SSLAnalyzer:
    ...
    def fetch_cipher_suite_info(self, cipher_suite):
        """Fetch detailed information about a cipher suite from ciphersuite.info with caching."""
        # Convert cipher suite name to the format expected by the API
        api_name = cipher_suite.replace('_', '-')
        encoded_name = quote(api_name)
        cache_file = os.path.join(self.cache_dir, f"{encoded_name}.json")
        ...
        if os.path.exists(cache_file):
            try:
                ...
                if time.time() - cache_data.get('timestamp', 0) < 86400:
                    return cache_data.get('data', {'Error': 'Invalid cache data'})
            ...
            if response.status_code == 200:
                ...
                if cs_data:
                    ...
                    with open(cache_file, 'w') as f:
                        json.dump({'timestamp': time.time(), 'data': details}, f)
                    
                    return details
            
            return {'Error': f"Unable to fetch cipher suite details for {cipher_suite}"}
